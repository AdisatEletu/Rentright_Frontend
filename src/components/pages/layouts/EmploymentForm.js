...
import { loadAllTenants, loadSpecificTenant, patchSpecificTenant, deleteSpecificTenant,showLoading, hideLoading, errorLoading  } from '../../../state/actions/tenantAction';
 ...
 class EmploymentForm extends Component{
    constructor(props) {           
      ...
      this.state = {};   
      try{        
      this.stateman = [[],...this.props.myProfile.tenants.tenant_employment_history];
  ...
  :
         ...
         <div className= "m-profile-setup t-flex t-flex-column">
            ...
            <div className= "t-gray-darken-3-f mid t-h1 t-flex t-flex-row t-justify-space-between t-align-top "><span className= "">Update Past Employment Records</span>
                  ...
                  <CircleLinks linkTo = {"/tenant/profile/generalinfo/" + this.props.match.params.id} scale = {true} childLabel = "General Info" label = "1" isActive = {false}/>
                   <CircleLinks  linkTo = {"/tenant/profile/bioinfo/" + this.props.match.params.id} scale = {true} childLabel = "Bio Info" label = "2" isActive = {false}/>                  
                       <CircleLinks linkTo = {"/tenant/profile/residentialinfo/" + this.props.match.params.id} scale = {true} childLabel = "Residential Info" label = "3" isActive = {false}/>
                          <CircleLinks linkTo = {"/tenant/profile/employmentinfo/" + this.props.match.params.id} scale = {false} childLabel = "Employment Info" label = "4" isActive = {true}/>
                </div>
             </div> 
    ...
    <div className="m-form-hold">       
        <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "textbox" size = "t-md-9" ownstate = {this.state.value} name = "employer"  initialvalue = "" icons = "verified_user" label = "Where did/do you work"/>     
     </div>
<div className = "major-padding col-md-10 space">
<div className = "m-heading increase">Do you Currently work here ? </div>
 <div className ="m-sub ">Please select if you work here now</div> 
<div  className = "t-flex t-flex-column t-justify-space-around t-align-top t-md-10">
<FormElements  onUpdate = {this.onUpdate.bind(this)} type = "radio" size = "s4" ownstate = {this.state}  name = "current_employment"  initialvalue = {false} icons = "verified_user" label = "Current Emplyer "/>
</div>
</div>{/* form wrapper*/} 
    <div className="m-form-hold">         
     <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "textbox" size = "t-md-9" ownstate = {this.state.value} name = "reasons_for_living"  initialvalue = "" icons = "" label = "If yes state reasons for leaving?"/>    
    </div>

    <div className="m-form-hold">         
     <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "date" size = "t-md-4" ownstate = {this.state.value} name = "employment_start"  initialvalue = "" icons = "" label = "When did you start working here"/>  
     <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "date" size = "t-md-4" ownstate = {this.state.value} name = "employment_ends"  initialvalue = "" icons = "" label = "When did you leave this job"/>   
    </div>
    <div className="m-form-hold">         
     <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "textbox" size = "t-md-4" ownstate = {this.state.value} name = "employer_telephone"  initialvalue = "" icons = "" label = "Telephone Number of your office"/>  
     <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "textbox" size = "t-md-4" ownstate = {this.state.value} name = "employment_email"  initialvalue = "" icons = "" label = "E mail of your office"/>   
    </div>
    <div className="m-form-hold">         
     <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "textbox" size = "t-md-4" ownstate = {this.state.value} name = "title"  initialvalue = "" icons = "" label = "What was your position while you worked here"/>  
     <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "textbox" size = "t-md-4" ownstate = {this.state.value} name = "address"  initialvalue = "" icons = "" label = "The office address in full"/>   
    </div>
      <div className="m-form-hold">         
     <FormElements  onUpdate = {this.onUpdate.bind(this)} type = "textbox" size = "t-md-9" ownstate = {this.state.value} name = "others"  initialvalue = "" icons = "" label = "Other relevant information"/>  
   </div>
